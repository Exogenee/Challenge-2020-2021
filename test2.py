...
for i in np.arange(1,9):
    dico[i] = pd.read_json(f'Visualisation{i}.json',lines=True)
...
for j in np.arange(0,dico[1].shape[0]):
    ...
    for i in np.arange(1,9):
        ...
        pt[0] = pt[1]
        pt[1] = a
        folium.CircleMarker(radius=dico[i]['intensity'][j]/50, location=pt, color="red", fill=True, fill_color="#3186cc", popup=f"Intensity:{dico[i]['intensity'][j]} Day:{str(dico[i]['Date'][j])[0:10]}").add_to(dict_mtp[j])
